...
from .nodes import split_train_val_test, train_classifier, eval_classifier, check_data
...
def create_pipeline(**kwargs) -> Pipeline:
    return pipeline([
        ...
        node(
            ...
            inputs=['model', 'X_train', 'y_train', 'X_val', 'y_val', 'X_test', 'y_test'],
            outputs=['clf_metrics', 'clf_figures'],
            name='eval_classifier',
            tags=['training']
        )
    ],
        inputs=['X', 'y'],
        outputs=['data_plots', 'model', 'clf_metrics', 'clf_figures'],
    )
