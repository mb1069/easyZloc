...
from data.visualise import show_psf_axial, concat_psf_axial, grid_psfs
...
import pandas as pd
def gaussian_window_smoothing(sharpness):
    smooth = pd.Series(sharpness).rolling(window=5, win_type='gaussian', center=True).mean(std=25)
    return np.argmax(smooth)

def measure_snr(psf):
    return psf.max() / np.median(psf)

def get_peak_sharpness(_psf, s=0.15, min_snr=0):
    ...
    snr = measure_snr(psf)
    if snr < min_snr:
        raise EnvironmentError(f'PSF snr is too low {round(snr, 3)}')
#     psf = butter_psf(_psf)
    ...
    #     # plt.xlabel('z slice')
    #     # ax0.legend()
    #     # sharpness_peak = np.argmax(sharpness)
    ...
    sharpness_peak = float(gaussian_window_smoothing(sharpness))
