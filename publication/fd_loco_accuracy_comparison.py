...
def load_ries_data(args):
    ...
    cols = list(ries_data)
    ries_deeploc = ries_data[[c for c in cols if ('DeepLoc' in c) or (
        'z(nm)' in c)]].dropna().set_index('z(nm)')
    crlb_deeploc = ries_data[[c for c in cols if ('CRLB' in c) or (
        'z(nm)' in c)]].dropna().set_index('z(nm)')
...
def plot_comparison(model_perf, ries_data, args, fname):
    ...
    for dataset, (z, error) in model_perf.items():
        ...
        sns.regplot(x=z, y=error, scatter=True, ci=95, order=25,
                    x_bins=np.arange(-1000, 1000, 50), label='Our method')
...
def parse_args():
    ...
    parser.add_argument(
        '--ries_data', default='/home/miguel/Projects/smlm_z/publication/comparisons/fd-loco/ries_comparison_data.csv')
