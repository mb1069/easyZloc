...
def aug_train_data(X_train, y_train, args):
    ...
    sample_psfs = X_train[0][idx].copy()
    aug_psfs = aug_pipeline(sample_psfs, training=True).numpy()
    ...
    n_samples = 200
    sample_images = sample_psfs[:n_samples].copy()  
...
def norm_images(X_train, X_val, X_test, args):
    ...
    datagen.fit(X_train[0], seed=args['seed'])
    print('Fitted')
...
def write_report(model, locs, train_data, val_data, test_data, args):
    ...
    for dirname, ds in [
        ...
        for num, c in tqdm(enumerate(set(coords2)), total=len(set(coords2))):
            ...
            fig = plt.figure(layout="constrained", figsize=(12, 10), dpi=40)
