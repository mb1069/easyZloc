...
def filter_beads(spots, locs, stacks, args):
    ...
    fwhms = np.array([has_fwhm(psf) for psf in stacks])

    idx = np.argwhere(snrs & fwhms).squeeze()
...
def write_combined_data(all_stacks, all_locs, args):
...
def main(args):
    ...
    write_combined_data(all_stacks, all_locs, args)
