...
def crop_z_view(locs, zrange=250):
    zs = locs['z [nm]']
    bin_width = 25
...
def align_x_axis(df):
    ...
    # plt.savefig(f'/home/miguel/Projects/smlm_z/publication/tmp.png')
    # print(f'/home/miguel/Projects/smlm_z/publication/tmp.png')
    # plt.close()
    # quit()
    df['x'] += x_mean
    df['y'] += y_mean
...
def write_nup_plots(locs, args, good_dir, other_dir):
    ...
    for cid in set(locs['clusterID']):
        ...
        if len(peak_scores_sorted) > 1:
            peak_scores_sorted = peak_scores_sorted[-1:]
        ...
        render_locs(df.copy(deep=True), args, (0, 0, 0),
                    barsize=1000, ax=ax1, viewport_margin=2)
        ax2 = fig.add_subplot(gs[0, 1])
        render_locs(df.copy(deep=True), args, (np.pi / 2, 0, 0),
                    barsize=500, ax=ax2, viewport_margin=2)

        ax3 = fig.add_subplot(gs[0, 2])
        render_locs(df.copy(deep=True), args, (0, np.pi / 2, 0),
                    barsize=500, ax=ax3, viewport_margin=2)
        # if color_by_depth:
