...
VERSION = '0.6'
CHANGE_NOTES = 'imsize 64, aug enabled with brightness 1 and no translation'
...
def split_train_val_test(beads, locs, ys):

    idx = np.arange(beads.shape[0])

    train_idx, test_idx = train_test_split(idx, train_size=0.9, random_state=args['seed'], stratify=locs['group'])
    train_idx, val_idx =  train_test_split(train_idx, train_size=0.9, random_state=args['seed'], stratify=locs.iloc[train_idx]['group'])

    train_beads = beads[train_idx]
    val_beads = beads[val_idx]
    test_beads = beads[test_idx]
    ...
    train_coords = locs[['x', 'y']].to_numpy()[train_idx]
    val_coords = locs[['x', 'y']].to_numpy()[val_idx]
    test_coords = locs[['x', 'y']].to_numpy()[test_idx]

    ds_cls = np.zeros((beads.shape[0]), dtype=object)
    ...
    train_psfs = np.concatenate(train_beads)
    val_psfs = np.concatenate(val_beads)
    test_psfs = np.concatenate(test_beads)

    train_coords = np.repeat(train_coords[:, :, np.newaxis], train_beads.shape[1], axis=0).squeeze()
    val_coords = np.repeat(val_coords[:, :, np.newaxis], val_beads.shape[1], axis=0).squeeze()
    test_coords = np.repeat(test_coords[:, :, np.newaxis], test_beads.shape[1], axis=0).squeeze()

    train_ys = np.concatenate(train_ys)[:, np.newaxis]
    val_ys = np.concatenate(val_ys)[:, np.newaxis]
    test_ys = np.concatenate(test_ys)[:, np.newaxis]

    print(train_psfs.shape)
    print(val_psfs.shape)
    print(test_psfs.shape)
...
def aug_train_data(X_train, y_train, args):
    ...
    aug_pipeline = Sequential([
        ...
        layers.RandomBrightness(0.1, [X_train[0].min(), X_train[0].max()], seed=args['seed']),
    ])
...
def write_report(model, train_data, val_data, test_data, args):
    ...
    for dirname, ds in [('train', train_data), ('val', val_data), ('test', test_data)]:
        ...
        preds = model.predict(ds, batch_size=N_GPUS * 256).squeeze()
...
def parse_args():
    ...
    parser.add_argument('-zrange', '--zrange', help='Z range to model [-val, +val] in nm', default=1000, type=int)
