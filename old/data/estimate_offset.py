...
def peak_to_stack_value(psf, peak, voxel_sizes):
    psf_shape = psf.shape
    x = np.linspace(0, psf_shape[0] - 1, num=psf_shape[0])
...
def estimate_offset(psf, voxel_sizes=voxel_sizes):
    ...
    if fit_failed or mean_squared_error(axial_max, gauss_func(x)) > 5e-3:
        ...
        # print(high_res_x)
        high_res_x = np.linspace(-int(target_psf_shape[0] * 0.25), int(target_psf_shape[0] * 1.25) - 1, 1000)
    ...
    return peak_to_stack_value(psf, peak, voxel_sizes)
